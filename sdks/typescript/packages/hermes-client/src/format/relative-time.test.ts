// Copyright Hermes Notifications
// SPDX-License-Identifier: Apache-2.0
// See LICENSE in the project root for license terms and DISCLAIMER.md for important usage information.

import { describe, expect, it } from "vitest";
import { relativeTime } from "./relative-time.js";

// The cases below are deliberately the same table as web/admin/lib/relative-time.test.ts,
// because this function is that one promoted into the SDK so the widget, the hooks and
// the admin portal stop carrying three subtly different implementations. Keeping the
// table identical is what makes "we adopted the admin semantics" a checked claim rather
// than an assertion in a commit message.
//
// The difference is the clock: the admin copy reads Date.now() internally and its test
// works around that with vi.useFakeTimers(). This one takes `now` as an argument, so
// there is nothing to stub.
const NOW = new Date("2026-07-28T12:00:00.000Z").getTime();

const SECOND = 1000;
const MINUTE = 60 * SECOND;
const HOUR = 60 * MINUTE;
const DAY = 24 * HOUR;

/** An ISO timestamp `ms` milliseconds before NOW. */
function ago(ms: number): string {
  return new Date(NOW - ms).toISOString();
}

describe("relativeTime", () => {
  it.each([
    { name: "reports the present as 'just now'", ms: 0, want: "just now" },
    // The four boundaries below are the ones the implementation's `<` comparisons sit
    // on. One second either side of each is what an off-by-one would move.
    { name: "still says 'just now' one second before a minute", ms: 59 * SECOND, want: "just now" },
    { name: "switches to minutes at exactly one minute", ms: MINUTE, want: "1m ago" },
    { name: "stays in minutes one minute before an hour", ms: 59 * MINUTE, want: "59m ago" },
    { name: "switches to hours at exactly one hour", ms: HOUR, want: "1h ago" },
    { name: "stays in hours one hour before a day", ms: 23 * HOUR, want: "23h ago" },
    { name: "switches to days at exactly one day", ms: DAY, want: "1d ago" },
    { name: "stays in days on the last day before a month", ms: 29 * DAY, want: "29d ago" },
    { name: "switches to months at exactly thirty days", ms: 30 * DAY, want: "1mo ago" },

    // Truncation, not rounding: 119 minutes is "1h ago", never "2h ago".
    { name: "truncates a partial hour downwards", ms: 119 * MINUTE, want: "1h ago" },
    { name: "truncates a partial day downwards", ms: 47 * HOUR, want: "1d ago" },

    // Months are fixed 30-day buckets, so a calendar year reports as 12mo.
    { name: "counts a year as twelve thirty-day months", ms: 365 * DAY, want: "12mo ago" },
  ])("$name", ({ ms, want }) => {
    expect(relativeTime(ago(ms), NOW)).toBe(want);
  });

  it("treats a future timestamp as 'just now' rather than going negative", () => {
    expect(relativeTime(new Date(NOW + HOUR).toISOString(), NOW)).toBe("just now");
  });

  // This is the bucket that changes versus the widget's old formatTime, which fell
  // through to toLocaleDateString() past 7 days. A locale-dependent string is not
  // something a test in CI can assert on, which is part of why this version wins.
  it("keeps counting days past a week instead of falling back to a locale date", () => {
    expect(relativeTime(ago(10 * DAY), NOW)).toBe("10d ago");
  });

  it("defaults to the current time when no clock is supplied", () => {
    expect(relativeTime(new Date().toISOString())).toBe("just now");
  });

  it("returns an empty string for an unparseable timestamp", () => {
    // The realtime payload's created_at is server-supplied, but a widget must not
    // render "NaNd ago" if it is ever malformed.
    expect(relativeTime("not a date", NOW)).toBe("");
  });
});
