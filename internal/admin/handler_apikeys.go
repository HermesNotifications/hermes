...
func (s *Server) registerAPIKeyRoutes() {
	...
	huma.Register(s.api, huma.Operation{
		...
		Description: "Replaces this key's rate limit. Omitted fields reset to the service default, " +
			"so an empty body clears the override entirely. This endpoint invalidates the key's " +
			"cache entry, so the new limit applies to the next bucket created for it — but a " +
			"caller that is continuously active keeps its current bucket, and therefore its old " +
			"limit, until it goes idle. Do not rely on this to throttle a caller mid-flood.",
		Tags: []string{"API Keys"},
