...
- **JetStream is a separate permission surface.** Publishing to a stream subject is not enough;
  ...
  `$JS.API.CONSUMER.MSG.NEXT`, `$JS.API.CONSUMER.INFO` and `$JS.ACK` for that one stream and
  consumer name. A missing JetStream grant looks like a broken stream, not a permissions problem —
  the request simply never gets a reply. `CONSUMER.INFO` is the quietest of them: it feeds the
  stall detector behind `HERMES_NATS_CONSUMER_STALL_TIMEOUT`, and without it the service runs
  normally with stall detection silently switched off (it logs `stall detection paused` when that
  happens, and `TestAccounts_EveryConsumerMayInspectItsOwnConsumer` guards the committed file).
