...
| `HERMES_DISPATCH_INSERT_BATCH_SIZE` | `1` (off) | How many notifications **dispatch** may persist in one database transaction. Raise it where the WAL flush is what limits the write path: with `synchronous_commit=on` a commit cannot return until its record is durable, so one transaction per notification caps throughput at roughly one notification per `fdatasync` regardless of cores or workers — which is where one measured environment landed (2,006 notifications/s against a volume doing 1,933 `fdatasync`/s, Postgres using 2.5 of 24 cores). **Off by default because it is a trade, not a free win:** batching also serialises commits the database could otherwise run in parallel, and against a volume doing 10,582 `fdatasync`/s it measured ~10% *slower*. Decide it with `pg_test_fsync` on the target volume and `cmd/dispatchbench -insert-batch 16`. The value is a ceiling rather than a target — batches are assembled only from rows already waiting while the previous transaction commits, so no notification waits for one that has not arrived — and the effective size is capped at `HERMES_DISPATCH_CONCURRENCY`, since every row in a batch is a worker blocked waiting for it. Watch `hermes.dispatch.insert.batch.size`: a mean near 1 means there is nothing to amortise. Note the insert is one of three commits per notification (the user upsert and the channel write-back are the others), so this can remove at best a third of the flushes. |
| `HERMES_DISPATCH_INSERT_BATCH_LINGER` | `0` (never wait) | How long an unfilled insert batch is held open waiting for more rows. Zero is the setting that scales: every row held is a worker that cannot start another notification, so a linger of X puts a ceiling of `concurrency / X` notifications per second on the service — at the default concurrency of 8, a 5ms linger caps dispatch at 1,600/s, below what it already does. Raise it only against a very slow volume, with a measurement in hand. |
